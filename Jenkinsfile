        ...
        stage('Deploy') {
            steps {
                script {
                    ...
                    bat """
                    docker ps -q --filter "ancestor=${env.DOCKER_IMAGE}" | findstr /r /v "^$" | for /F "tokens=*" %%i in ('more') do docker stop %%i
                    docker ps -a -q --filter "ancestor=${env.DOCKER_IMAGE}" | findstr /r /v "^$" | for /F "tokens=*" %%i in ('more') do docker rm %%i
                    """
